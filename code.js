...
try {
    ...
    if (figma.currentPage.selection[0].type === 'INSTANCE' &&
        ...
        figma.notify(`Your current selection is a ${figma.currentPage.selection[0].children[0].type}. Please choose a vector or rectangle.`);
        figma.closePlugin();
    }
    if (figma.currentPage.selection[0].type !== 'VECTOR' &&
        ...
        figma.notify(`Your current selection is a ${figma.currentPage.selection[0].type}. Please choose a vector or rectangle.`);
        figma.closePlugin();
    }
        ...
        figma.ui.on('message', uiResponse => {
            ...
            if (uiResponse.type === 'convertSelectedArtboard') {
                if (uiResponse.selectedArtboard.length !== 0) {
                    ...
                    angleArtboard(node, selectedNode, uiResponse.orientation, uiResponse.pixel, uiResponse.quality, getCoordinates(node), node.width, node.height);
                }
            }
        ...
        figma.ui.on('message', uiResponse => {
            try {
                ...
                if (uiResponse.type === 'convertSelectedArtboard') {
                    if (uiResponse.selectedArtboard.length !== 0) {
                        const selectedNode = findSelectedNode(uiResponse.selectedArtboard);
                        angleArtboard(figma.currentPage.selection[0], selectedNode, uiResponse.orientation, uiResponse.pixel, uiResponse.quality, getCoordinates(figma.currentPage.selection[0]), figma.currentPage.selection[0].width, figma.currentPage.selection[0].height);
                    }
                }
            }
            catch (error) { }
            ...
            figma.ui.on('message', uiResponse => {
                try {
                    ...
                    if (uiResponse.type === 'convertSelectedArtboard') {
                        if (uiResponse.selectedArtboard.length !== 0) {
                            const selectedNode = findSelectedNode(uiResponse.selectedArtboard);
                            angleArtboard(convertedNode, selectedNode, uiResponse.orientation, uiResponse.pixel, uiResponse.quality, getCoordinates(convertedNode), convertedNode.width, convertedNode.height);
                        }
                    }
                }
                catch (error) { }
    ...
    if (figma.currentPage.selection[0].parent.parent.parent.type === 'INSTANCE' &&
        figma.currentPage.selection[0].type === 'RECTANGLE') {

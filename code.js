...
try {
    ...
    figma.currentPage.children.map(function (node) {
        if (node.type === "FRAME") {
            ...
            const filterSelection = nodesNames.filter(nodesNames => nodesNames !== getRootFrame(currentUserSelection));
            ...
            if (uiResponse.type === "convertSelectedArtboard") {
                if (uiResponse.selectedArtboard.length !== 0) {
                    ...
                    if (currentUserSelection.type === "RECTANGLE") {
                        ...
                        figma.closePlugin();
                        figma.notify(`Your current selected screen is a ${currentUserSelection.type} node. Please choose a Vector node instead`);
                    }

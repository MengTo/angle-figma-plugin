...
try {
    ...
    if (figma.currentPage.selection[0].type === 'RECTANGLE' &&
            ...
            */
            figma.ui.on('message', uiResponse => {
                ...
                if (uiResponse.type === 'convertSelectedArtboard') {
                    if (uiResponse.selectedArtboard.length !== 0) {
                        const selectedNode = findSelectedNode(uiResponse.selectedArtboard);
                        angleArtboard(vectorNode, selectedNode, uiResponse.orientation, uiResponse.pixel, uiResponse.quality, getCoordinates(vectorNode), vectorNode.width, vectorNode.height);
                    }
                }
                if (uiResponse.type === 'networkResponse') {
                    // uiResponse.response
    ...
    else if (figma.currentPage.selection[0].type === 'RECTANGLE' &&
        ...
        figma.ui.on('message', uiResponse => {
            ...
            if (uiResponse.type === 'networkResponse') {
                // uiResponse.response
                const selectedImage = angleFill(uiResponse.response, figma.currentPage.selection[0].children[0]);
                console.log(selectedImage);
